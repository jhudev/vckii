#' Fetch specific hashes from a dataset
#'
...
#' @param data Working dataset for hashing.
...
fetchHash.data <- function(data,t="all"){
  ...
  #HASH GEN++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  #If type is "all","dat","cell+col","cell+row"
  if(t=="all") return(paste(.pFetchHash_all(data),.pFetchHash_colName(data),.pFetchHash_rowName(data),sep="-"))
  if(t=="cell") return(.pFetchHash_all(data))
  if(t=="cell+col") return(paste(.pFetchHash_all(data),.pFetchHash_colName(data),sep="-"))
  if(t=="cell+row") return(paste(.pFetchHash_all(data),"X",.pFetchHash_rowName(data),sep="-"))

  #If type is 'col' or 'colValue'
  if(t %in% c("col","colValue")){
    hashIDList_col <- .pFetchHash_col(data)
    if(t=="colValue") return(hashIDList_col)
    return(.pFetchHash_list(hashIDList_col))
  }

  #If type is 'row' or 'rowValue'
  if(t %in% c("row","rowValue")){
    hashIDList_row <- .pFetchHash_row(data)
    if(t=="rowValue") return(hashIDList_row)
    return(.pFetchHash_list(hashIDList_row))
  }

  #If type is 'colName' or 'rowName'
  if(t=="colName"){return(.pFetchHash_colName(data))}
  if(t=="rowName"){return(.pFetchHash_rowName(data))}
}
