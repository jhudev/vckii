#' Fetch specific hashes from a hash file
#'
#' This function gets specific hashes from a hash file.
#' Can be used for manual debugging with fetchHash.data()
#' @param file A character string naming the file and (path) directory of the hash key.
#' @param t the type of hash(es) to return. Options: all (default), cell, cell+col, cell+row, row, rowValue [array], col, colValue [array], colName, rowName
#' @keywords hash, key, fetch
#' @export
#' @examples
...
fetchHash.file <- function(file,t="all"){
  #Check if inputs are correct
  if(!file.exists(file)) stop("'file' does not exist.")
  if(length(t)!=1) stop("'type' (t) must be a single character string")
  if(!(t %in% c("all","cell","cell+col","cell+row","col","colValue","row","rowValue","colName","rowName"))){
    stop("'type' (t) must be: all, cell, cell+col, cell+row, col, colValue, row, rowValue, colName, rowName")}
  ...
  #If type is "all","cell","cell_col","cell_row"
  hashChkStats_main <- strsplit(hashChkStats[2],split=" ")[[1]] #< fetch data, colNames, rowNames hash (2 row)
  if(t=="all") return(paste(hashChkStats_main,collapse="-"))
  if(t=="cell") return(hashChkStats_main[1])
  if(t=="cell+col") return(paste(hashChkStats_main[1],hashChkStats_main[2],sep="-"))
  if(t=="cell+row") return(paste(hashChkStats_main[1],"X",hashChkStats_main[3],sep="-"))

  #If type is "col","colValue","row","rowValue"
  hashChkDim <- as.numeric(strsplit(hashChkStats[1],split=" ")[[1]]) # <- fetch dimension (1 row)
  hashChkStats_sec <- strsplit(hashChkStats[3],split=" ")[[1]] #< fetch col and row single hash (3 row)
  if(t=="col") return(hashChkStats_sec[1])
  if(t=="colValue") return(hashChkAll[5:(4+hashChkDim[1])])
  if(t=="row") return(hashChkStats_sec[2])
  if(t=="rowValue") return(hashChkAll[(4+hashChkDim[1]+2):(4+hashChkDim[1]+1+hashChkDim[2])])

  #If type is 'colName','rowName'
  if(t=="colName") return(hashChkStats_main[2])
  if(t=="rowName") return(hashChkStats_main[3])
}
